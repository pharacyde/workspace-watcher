...
- **No Lombok, deliberately.** It was considered and measured: version 1.18.46, the latest, silently
  generated nothing on JDK 26 - `@Getter` compiles and the getter simply does not exist. It works on
  17 and 21, so this is a JDK-26 incompatibility, not a configuration mistake. Development has
  since moved to JDK 27 and the measurement has not been repeated there; the argument below is
  the reason it need not be. Lombok hooks into
